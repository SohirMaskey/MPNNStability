...
from Settings import radiussteps, datapath
...
class RGGDataset_grid(Dataset):
    ...
    def raw_file_names(self):
        ...
        """
        return [f'graph_r{round(radius * radiussteps)}_{int(i*self.skip)}.pt' for i in range(1, self.number)]
    
    @property
    def processed_file_names(self):
        """ If these files are found in processed_dir, processing is skipped"""
        
        return [f'graph_r{round(self.radius * radiussteps)}_{int(i * self.skip)}.pt' for i in range(1, self.number)]
    ...
    def process(self): 
        ...
        for i in range(1, self.number+1):
            #print(i)
            #print(len(positions[i-1][0]))
            batch = torch.zeros(round(i*self.skip)).type(torch.LongTensor)
...
if __name__=='__main__':
    DL = RGGDataset_grid(root = datapath, size = 7)
    DL.download_grid()
    DL.process_grid()
